...
class InputFeature(object):
    def __init__(self, qid, ids_dict, offset_mapping, context, piece_idxes, sep_poses,\
         relations, utterance_ids_dict, speaker_ids_dict, start_pos=None, end_pos=None, is_impossible=None):
        self.qid = qid
...
def convert_examples_to_features(examples, tokenizer, speaker_mask_path, max_length, training=True, max_utterance_num=args.max_utter_num):
    ...
    def _get_utterance_gather_ids(input_ids, utterance_num):
        ...
        return utterance_gather_ids, utterance_p_mask
    ...
    for exp in tqdm(examples, ncols=100):
        ...
        context = tokenizer.pad_token + ' ' + context # add a [PAD] for padding utterances
        ...
        utterance_gather_ids, utterance_p_mask = _get_utterance_gather_ids(input_ids, len(exp.utterances))
        ...
        if not training:
            ...
            f_tmp = InputFeature(exp.qid, ids_dict, offset_mapping, context,\
                 piece_idxes, sep_poses, exp.relations, utterance_ids_dict, speaker_ids_dict)
        ...
        f_tmp = InputFeature(exp.qid, ids_dict, offset_mapping, context, piece_idxes, sep_poses,\
             exp.relations, utterance_ids_dict, speaker_ids_dict, start_pos, end_pos, is_impossible)
        features.append(f_tmp)
        max_tokens = max(max_tokens, text_len)

    print("Max token length:", max_tokens)
    print("Speaker negative rate: %.2f%%" %(neg_num / all_num * 100))
...
def get_dataset(input_file, save_path, tokenizer, max_length, training=True):
    ...
    if not os.path.exists(feature_path):
        features = convert_examples_to_features(examples, tokenizer, speaker_mask_path, max_length, training=training)
        if not args.colab:
...
if __name__ == "__main__":
    ...
    speaker_mask_path = "data/speaker_mask_{}.json".format(input_file.split('/')[1].split('.')[0])
