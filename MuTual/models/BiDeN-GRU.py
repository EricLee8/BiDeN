...
class MultipleChoiceModel(pretrained_model_class):
    ...
    def forward(
        ...
        # summarize the sequence using simple max-mean_pooling
        ...
        for i in range(bsz):
            for j in range(self.n_choices):
                ...
                utter_pooler = torch.mean(utters_tensor, dim=0) # (hsz)
                batch_utter_emb_list.append(utter_pooler)
        summary_vec = torch.stack(batch_utter_emb_list, dim=0).view(-1, self.n_choices, self.hidden_size) # (bsz*n_choices, hsz)
